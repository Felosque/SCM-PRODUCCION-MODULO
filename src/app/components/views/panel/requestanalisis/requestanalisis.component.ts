...
export class RequestanalisisComponent implements OnInit {
  ...
  constructor(
    private _requestsAnalisisService: RequestanalysisService
  ) {
    this._requestsAnalisisService.lisrequestanalysis().subscribe(
      (b: string) => {
  ...
  showrequestanalisis(){
    this._requestsAnalisisService.getRequestanalysis().subscribe(response =>{
